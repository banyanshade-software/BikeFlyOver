...
Render the fly-over on real 3D terrain so mountains, valleys, and hills are visible in preview/export, while letting the user exaggerate terrain relief with a bounded vertical scale control.
...
- There is no user control for terrain exaggeration.
...
2. Add a user-facing terrain exaggeration parameter:
...
- Add a terrain exaggeration parameter and expose it in the sidebar/export-shared settings flow.
...
- Preview shows real 3D terrain relief instead of a flat imagery-only globe.
